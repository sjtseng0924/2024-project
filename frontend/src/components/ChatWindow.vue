...
import {
  getAllMessages,
  sendMessage,
} from "@/services/chatroom/chatroomService";
...
export default {
  ...
  methods: {
    async fetchMessages(shouldScroll = false) {
      try {
    ...
    async send() {
      ...
      try {
        ...
        this.newMessage = "";
        this.fetchMessages(true); // Trigger fetch and scroll to bottom after sending
      } catch (error) {
        console.error(error);
      } finally {
        this.sending = false;
  ...
  mounted() {
    this.fetchMessages(true); // Fetch messages and scroll to bottom initially
    this.intervalId = setInterval(() => this.fetchMessages(false), 3000); // Fetch messages every 3 seconds without scrolling
  },
  beforeUnmount() {
